...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="p-1.5 rounded-lg hover:bg-teal/10 transition-colors"
      title="Copy"
        ...
        [
          {
            role: "system",
            content:
              "You are an expert content creator and digital marketing specialist for Quick Bee AI Growth Engine.",
          },
  ...
  const parseContent = (raw: string): ContentSections => {
    const extract = (marker: string, nextMarker?: string) => {
      ...
      const end = nextMarker
        ? raw.indexOf(nextMarker, contentStart)
        : raw.length;
    ...
    return {
      seoBlog:
        extract("## SEO BLOG POST", "## SOCIAL MEDIA CAPTIONS") ||
        raw.slice(0, raw.length / 4),
      socialCaptions:
        extract("## SOCIAL MEDIA CAPTIONS", "## LINKEDIN POST") || "",
  ...
  const sections = content
    ? [
        { key: "seoBlog", label: "SEO Blog Post", text: content.seoBlog },
        {
          ...
          text: content.socialCaptions,
        },
        {
          key: "linkedinPost",
          label: "LinkedIn Post",
          text: content.linkedinPost,
        },
        {
          key: "instagramCarousel",
          label: "Instagram Carousel Outline",
          text: content.instagramCarousel,
        },
      ]
    : [];
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="page-subtitle">
              Generate SEO blogs, social captions, LinkedIn posts & Instagram
              carousels
            </p>
          </div>
        </div>
      ...
      <div className="glass-card rounded-xl p-5">
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            disabled={loading || !isConfigured || !keyword.trim()}
        ...
        <>
          <div className="flex justify-end">
            <Button
              ...
              variant="ghost"
              className="border border-teal/20 text-white/60 hover:text-teal"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {sections.map(({ key, label, text }) => (
              <div key={key} className="glass-card rounded-xl p-5">
                ...
                <div
                  className="text-xs whitespace-pre-wrap max-h-64 overflow-y-auto scrollbar-thin"
                  style={{ color: "rgba(232,245,244,0.7)", lineHeight: "1.6" }}
                >
