...
import { AdPlatform, PostStatus, SocialMediaPlatform } from "../backend";
...
function getHealthStatus(
  ...
  if (ctr >= thresholds.minCTR && conversions >= thresholds.minConversions)
    return "Healthy";
  if (
    ctr >= thresholds.minCTR * 0.5 ||
    conversions >= thresholds.minConversions * 0.5
  )
    return "At Risk";
...
export function CampaignAutopilotPage() {
  ...
  const [newWindowPlatform, setNewWindowPlatform] =
    useState<SocialMediaPlatform>(SocialMediaPlatform.instagram);
  ...
  const [thresholdCTR, setThresholdCTR] = useState(
    String(config.healthThresholds.minCTR),
  );
  const [thresholdConversions, setThresholdConversions] = useState(
    String(config.healthThresholds.minConversions),
  );
  ...
  const handleSaveThresholds = () => {
    const ctr = Number.parseFloat(thresholdCTR) || 0;
    const conv = Number.parseInt(thresholdConversions) || 0;
    const newConfig = updateHealthThresholds({
      minCTR: ctr,
      minConversions: conv,
    });
    setConfig(newConfig);
  ...
  const handleGenerateSummary = () => {
    ...
    const totalConversions = adCampaigns.reduce(
      (acc, c) => acc + Number(c.conversions),
      0,
    );
    const avgCTR =
      totalImpressions > 0
    ...
    const topPlatform =
      Object.entries(byPlatform).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "N/A";

    const healthCounts = { Healthy: 0, "At Risk": 0, Underperforming: 0 };
    for (const d of campaignHealthData) {
      healthCounts[d.status]++;
    }
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">
            Auto-schedule content, monitor campaign health, and generate
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="glass-card rounded-xl p-5 space-y-4">
          ...
          <p className="text-xs text-white/40">
            Define optimal posting windows per platform. Scheduled posts will be
            sorted accordingly.
          </p>
          ...
          <div className="flex flex-wrap gap-2 items-end">
            <div className="space-y-1">
              ...
              <Select
                value={newWindowPlatform}
                onValueChange={(v) =>
                  setNewWindowPlatform(v as SocialMediaPlatform)
                }
              >
                ...
                <SelectContent className="bg-[#0a1010] border-teal/20 text-white">
                  {Object.values(SocialMediaPlatform).map((p) => (
                    <SelectItem
                      key={p}
                      value={p}
                      className="focus:bg-teal/10 focus:text-teal text-xs"
                    >
                      {PLATFORM_LABELS[p]}
                    </SelectItem>
                  ))}
          ...
          {config.postingWindows.length > 0 ? (
            <div className="space-y-1.5">
              {config.postingWindows.map((w, i) => (
                <div
                  // biome-ignore lint/suspicious/noArrayIndexKey: posting windows have no stable ID
                  key={i}
            ...
            <p className="text-xs text-white/25 text-center py-3">
              No posting windows configured yet
            </p>
          )}
                  ...
                  >
                    ...
                    {post.scheduledDate && (
                      <span className="text-xs text-white/30 flex-shrink-0">
                        {new Date(
                          Number(post.scheduledDate) / 1_000_000,
                        ).toLocaleDateString()}
                      </span>
                    )}
                  </div>
        ...
        <div className="glass-card rounded-xl p-5 space-y-4">
          ...
          <div className="bg-white/3 rounded-lg p-3 space-y-3">
            ...
            <div className="flex flex-wrap gap-3 items-end">
              <div className="space-y-1">
                <span className="text-xs text-white/40">Min CTR (%)</span>
                <Input
                  type="number"
                  min="0"
                  step="0.1"
              ...
              <div className="space-y-1">
                <span className="text-xs text-white/40">Min Conversions</span>
                <Input
                  type="number"
                  min="0"
          ...
          {adsLoading ? (
            ...
            <div className="text-center py-8 text-white/20">
              ...
              <p className="text-xs mt-1">
                Add campaigns in the Ads Tracker to see health status
              </p>
            </div>
          ) : (
              ...
              {campaignHealthData.map(({ campaign, ctr, status }) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white/40">
                      {AD_PLATFORM_LABELS[campaign.platform]} · CTR:{" "}
                      {ctr.toFixed(2)}% · Conv:{" "}
                      {Number(campaign.conversions).toLocaleString()}
                    </p>
                  </div>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full border flex-shrink-0 ${HEALTH_STYLES[status].badge}`}
                  >
                    {status}
                  </span>
                </div>
          ...
          {campaignHealthData.length > 0 && (
            <div className="flex gap-2 flex-wrap">
              {(
                ["Healthy", "At Risk", "Underperforming"] as HealthStatus[]
              ).map((s) => {
                const count = campaignHealthData.filter(
                  (d) => d.status === s,
                ).length;
                return (
                  <div
                    key={s}
                    className={`flex items-center gap-1.5 text-xs px-2 py-1 rounded-full border ${HEALTH_STYLES[s].badge}`}
                  >
      ...
      <div className="glass-card rounded-xl p-5 space-y-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  handleSummaryScheduleChange(v as SummarySchedule)
                }
              >
        ...
        {config.lastSummaryGenerated && (
          <p className="text-xs text-white/30">
            Last generated:{" "}
            {new Date(config.lastSummaryGenerated).toLocaleString()}
          </p>
        )}
        ...
        {summaryGenerated && summary ? (
          ...
          <div className="text-center py-10 text-white/20 border border-dashed border-white/10 rounded-lg">
            ...
            <p className="text-sm">
              Click "Generate Summary" to create a performance report
            </p>
            <p className="text-xs mt-1">
              Aggregates data from all your ad campaigns and scheduled posts
            </p>
          </div>
        )}
      </div>
