...
const quickActions = [
  ...
  },
  {
    ...
    icon: Settings2,
    desc: "Create & manage services",
  },
  {
    path: "/ai-tools",
    label: "AI Smart Systems",
    icon: Bot,
    desc: "6 AI-powered tools",
  },
  {
    path: "/settings",
    label: "Sales Config",
    icon: Settings,
    desc: "Configure credentials",
  },
  {
    path: "/automation",
    label: "Automation",
    icon: Zap,
    desc: "Toggle automations",
  },
  {
    path: "/workflows",
    label: "Workflows",
    icon: GitBranch,
    desc: "Run automation workflows",
  },
  {
    path: "/analytics",
    label: "Analytics Engine",
    icon: BarChart3,
    desc: "GA4-style reports",
  },
  {
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(8,12,12,0.85) 0%, rgba(0,180,166,0.15) 100%)",
          }}
        />
        <div className="relative z-10 p-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            />
            <div>
              <h1 className="text-3xl font-display font-bold teal-gradient-text">
                Quick Bee AI Growth Engine
              </h1>
              <p style={{ color: "rgba(232,245,244,0.6)" }} className="text-sm">
                Your all-in-one AI agency platform dashboard
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card rounded-xl p-5">
          ...
          {logs.length === 0 ? (
            <p className="text-sm" style={{ color: "rgba(232,245,244,0.4)" }}>
            ...
            <div className="space-y-2">
              {logs.map((log) => (
                ...
                >
                  <div
                    className={`w-2 h-2 rounded-full flex-shrink-0 ${log.success ? "bg-green-400" : "bg-red-400"}`}
                  />
                  ...
                  <div
                    className={`text-xs font-mono flex-shrink-0 ${log.success ? "text-green-400" : "text-red-400"}`}
                  >
                    {log.statusCode}
                  </div>
                </div>
        ...
        <div className="glass-card rounded-xl p-5">
          ...
          <div className="space-y-3">
            {automationItems.map(({ key, label }) => {
              const isOn =
                automationConfig[key as keyof typeof automationConfig];
              return (
                <div key={key} className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded-full ${isOn ? "bg-teal/15 text-teal" : "bg-white/5 text-white/30"}`}
                    style={
                      isOn
                        ? {
                            color: "#00d4c8",
