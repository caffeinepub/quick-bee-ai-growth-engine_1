...
export function LandingPagesPage() {
  const { data: pages = [], isLoading } = useGetAllLandingPages();
  ...
  const activeCount = pages.filter(
    (p) => p.status === LandingPageStatus.active,
  ).length;
  const pausedCount = pages.filter(
    (p) => p.status === LandingPageStatus.paused,
  ).length;
  const draftCount = pages.filter(
    (p) => p.status === LandingPageStatus.draft,
  ).length;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">
            Track and manage your marketing landing pages
          </p>
        </div>
        <Button
          onClick={openCreate}
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {pages.map((page) => (
            <div
              key={String(page.id)}
              className="glass-card rounded-xl p-4 hover:border-teal/30 transition-colors"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border ${STATUS_STYLES[page.status]}`}
                    >
                  ...
                  {page.url && (
                    ...
                    >
                      <ExternalLink
                        size={12}
                        className="text-teal/60 group-hover:text-teal transition-colors flex-shrink-0"
                      />
                      <span className="text-xs text-teal/60 group-hover:text-teal transition-colors truncate max-w-xs">
                        {page.url}
                      </span>
                    </a>
                  )}
