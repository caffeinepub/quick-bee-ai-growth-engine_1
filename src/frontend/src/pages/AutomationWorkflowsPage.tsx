...
const workflows: Workflow[] = [
  {
    ...
    description:
      "Triggers when a new lead is added to the CRM. Sends notifications and starts follow-up sequence.",
    icon: UserPlus,
    color: "#60a5fa",
    payload: {
      workflow: "new-lead-submission",
      event: "lead.created",
      data: { source: "manual-trigger", timestamp: new Date().toISOString() },
    },
  },
  {
    id: "meeting",
    ...
    description:
      "Handles meeting scheduling workflows including calendar invites and reminder sequences.",
    icon: Calendar,
    color: "#c084fc",
    payload: {
  ...
  },
  {
    ...
    description:
      "Processes payment confirmations, generates invoices, and triggers onboarding workflows.",
    icon: CreditCard,
    color: "#4ade80",
    payload: {
  ...
  },
  {
    ...
    description:
      "Runs analytics data collection and generates performance reports.",
    icon: BarChart3,
    color: "#00d4c8",
    payload: {
      workflow: "analytics-engine",
      event: "analytics.run",
      data: { source: "manual-trigger", dateRange: "last-30-days" },
    },
  },
  {
    id: "content",
    ...
    description:
      "Triggers AI content generation pipeline for scheduled social media and blog posts.",
    icon: PenTool,
    color: "#fbbf24",
    payload: {
      workflow: "ai-content-creation",
...
export function AutomationWorkflowsPage() {
  const [states, setStates] = useState<Record<string, WorkflowState>>(
    Object.fromEntries(
      workflows.map((w) => [
        w.id,
        { status: "idle" as WorkflowStatus, result: null },
      ]),
    ),
  );
  ...
  const runWorkflow = async (workflow: Workflow) => {
    ...
    setStates((prev) => ({
      ...prev,
      [workflow.id]: { ...prev[workflow.id], status: "running" },
    }));
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="page-subtitle">
              Manually trigger and monitor automation workflows
            </p>
          </div>
        </div>
      </div>
