...
import { useDeleteAdCampaign, useGetAllAdCampaigns } from "../hooks/useQueries";
...
export function AdsTrackerPage() {
  const { data: campaigns = [], isLoading } = useGetAllAdCampaigns();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">
            Track your paid advertising campaigns across all platforms
          </p>
        </div>
        <Button
          onClick={openCreate}
      ...
      <div className="flex items-center gap-3">
        <Select value={filterPlatform} onValueChange={setFilterPlatform}>
          ...
          <SelectContent className="bg-[#0a1010] border-teal/20 text-white">
            <SelectItem
              value="all"
              className="focus:bg-teal/10 focus:text-teal"
            >
              All Platforms
            </SelectItem>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-white/30">
          ...
          <p className="text-sm mt-1">
            Create your first ad campaign to start tracking performance
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left py-3 px-3 text-white/40 font-medium">
                  Campaign
                </th>
                <th className="text-left py-3 px-3 text-white/40 font-medium">
                  Platform
                </th>
                <th className="text-right py-3 px-3 text-white/40 font-medium">
                  Budget
                </th>
                <th className="text-right py-3 px-3 text-white/40 font-medium">
                  Spend
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-3">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full border ${PLATFORM_STYLES[campaign.platform]}`}
                      >
                        {PLATFORM_LABELS[campaign.platform]}
                      </span>
                    </td>
                    <td className="py-3 px-3 text-right text-white/70">
                      {fmtCurrency(campaign.budget)}
                    </td>
                    <td className="py-3 px-3 text-right text-white/70">
                    ...
                    <td className="py-3 px-3">
                      <div className="flex gap-1 justify-end">
                        <button
                          ...
                          className="p-1.5 text-white/40 hover:text-teal transition-colors"
                        >
                          <Pencil size={13} />
                        </button>
                        <button
        ...
        <AlertDialogContent className="bg-[#0a1010] border border-teal/20 text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/50">
              Are you sure you want to delete "{deleteTarget?.campaignName}"?
              This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-white/5 border-teal/20 text-white hover:bg-white/10">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
