...
const STATUS_STYLES: Record<Variant_active_sent_draft, string> = {
  [Variant_active_sent_draft.draft]:
    "bg-orange-500/20 text-orange-300 border-orange-500/30",
  [Variant_active_sent_draft.active]:
    "bg-teal-500/20 text-teal-300 border-teal-500/30",
  [Variant_active_sent_draft.sent]:
...
export function EmailCampaignsPage() {
  const { data: campaigns = [], isLoading } = useGetAllEmailCampaigns();
  ...
  const draftCount = campaigns.filter(
    (c) => c.status === Variant_active_sent_draft.draft,
  ).length;
  const activeCount = campaigns.filter(
    (c) => c.status === Variant_active_sent_draft.active,
  ).length;
  const sentCount = campaigns.filter(
    (c) => c.status === Variant_active_sent_draft.sent,
  ).length;
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border ${STATUS_STYLES[campaign.status]}`}
                    >
