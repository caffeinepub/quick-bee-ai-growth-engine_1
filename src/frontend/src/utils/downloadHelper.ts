...
export function downloadAsExcel(data: unknown[], filename: string): void {
  ...
  const headerCells = allHeaders
    .map(
      (h) =>
        `<th style="background:#00d4c8;color:#fff;font-weight:bold;padding:6px 10px;border:1px solid #ccc;">${escapeXml(h)}</th>`,
    )
    .join("");
  const dataRowsHtml = flatRows
    .map((row) => {
      const cells = allHeaders
        .map(
          (h) =>
            `<td style="padding:5px 10px;border:1px solid #ddd;">${escapeXml(row[h] ?? "")}</td>`,
        )
        .join("");
      return `<tr>${cells}</tr>`;
    })
...
export function downloadAsPDF(
  ...
  const emptyMsg =
    data.length === 0
      ? '<p class="empty">No data available to export.</p>'
      : "";
