...
export function getCampaignAutopilotConfig(): CampaignAutopilotConfig {
  try {
    ...
    return {
      ...getDefaultConfig(),
      ...JSON.parse(raw),
    } as CampaignAutopilotConfig;
  } catch {
    return getDefaultConfig();
  }
}

export function saveCampaignAutopilotConfig(
  config: CampaignAutopilotConfig,
): void {
  try {
...
export function addPostingWindow(
  window: PostingWindow,
): CampaignAutopilotConfig {
...
export function updateHealthThresholds(
...
export function sortPostsByPostingWindows<
  T extends { scheduledDate?: bigint; platform: SocialMediaPlatform },
>(posts: T[], windows: PostingWindow[]): T[] {
